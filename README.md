...
Примечание: СберЗдоровье (docdoc.ru, sberhealth.ru) защищено ServicePipe. Обычные HTTP-запросы (в том числе curl_cffi) получают страницу JS-проверки вместо страницы врача, поэтому коллектор автоматически переключается на браузер Camoufox, который проходит проверку и сохраняет cookies в `BROWSER_PROFILE_DIR`. Camoufox обязателен: установите зависимости из `requirements.txt` и выполните `camoufox fetch`. Если с IP сервера появляется капча, используйте `BROWSER_HEADLESS=virtual`/`false` для ручного прохождения или `HTTPS_PROXY`.
